...
let tags: string[] = initialStorage ? JSON.parse(initialStorage) : [];

// TextFieldを作成
const createTextfield = (): HTMLInputElement => {
  const textfield = document.createElement("input");
  textfield.setAttribute("type", "text");
  textfield.setAttribute(
    "class",
    "bg-color-twitter-white txt-size--14 padding-a--0 compose-textfield"
  );
...
// TextFieldをラップして埋め込む
const appendTextfield = (textfield: HTMLInputElement) => {
  const wrapper = document.createElement("div");
  wrapper.setAttribute(
    "class",
    "position-rel compose-text-container padding-a--10 br--4 margin-b--12"
  );
...
// タグボタン(1個)の作成
const createTagButton = (tagText: string): HTMLButtonElement => {
  const button = document.createElement("button");
  button.setAttribute(
    "class",
    "btn btn-on-blue full-width txt-left margin-b--12 padding-v--6 padding-h--12 tag-button"
  );
...
// textareaにタグを入力
const putTagInTextarea = (tag: string) => {
  const textarea = document.getElementsByClassName(
    "compose-text"
